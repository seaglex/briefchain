...
- [x] 2.3 Call `POST /api/v1/briefs/{brief_id}/transfer?action=send` with `is_temporary_user=true` when the temporary user form is submitted.
...
- [x] 4.2 Pass a flag to hide creator-only actions (send, edit, review, cancel, suspend, resume, approve, reject_submit, update) when rendered in invite view mode.
- [x] 4.3 Render invite-specific action controls (accept, reject, submit, block, open, delegate) below the Brief detail based on the current `upstream_state` / `downstream_state`.
...
- [x] 6.2 Open the invite link in an incognito window and verify the header, Brief detail, accept/reject, and downstream actions.
