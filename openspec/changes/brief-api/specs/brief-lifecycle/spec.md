...
- **THEN** the system sets `downstream_state` to "opened", leaves `upstream_state` as "in_process", does NOT modify `current_version` or any `BriefVersion.status`, updates `status_changed_at` / `status_changed_by`, creates a feedback record with `type` "reject_submit" and `is_to_down` true, and returns the updated brief
...
- **THEN** the system
  - creates a new version v(n+1) and transitions it through `draft` → `reviewed` → `sent` (MVP auto-pass),
  - updates `briefs.current_version` to the new version and synchronizes `briefs.title` / `priority` / `expected_completion_at` to the sent version,
  - leaves `briefs.upstream_state` as "in_process",
