...
- **THEN** the system returns the brief with its current sent version content, `version` equals `current_version`, `is_current` is `true`, and `unsent_version` is the version number of any editable draft if one exists, otherwise `null`
...
- **THEN** the system returns the brief with version 1 content, `version` equals 1, `is_current` is `true`, and `unsent_version` is 2

#### Scenario: Get historical version
- **WHEN** an authenticated user sends a GET request to `/api/v1/briefs/:brief_id?version=1`
- **THEN** the system returns the brief content for version 1, `is_current` is `true` only when version 1 equals `current_version`, and `unsent_version` is the version number of any editable draft if one exists, otherwise `null`
...
- **THEN** the system updates the existing draft version in place, does NOT synchronize `briefs.title` / `priority` / `expected_completion_at` (only `send` or `update` updates those denormalized fields), and returns the updated brief with the patched version number and `unsent_version` set to that version number
...
- **THEN** the system creates a new draft version with the next version number, leaves `current_version` pointing to the old sent version, leaves `briefs.title` / `priority` unchanged, and returns the updated brief with `unsent_version` set to the new draft version number
