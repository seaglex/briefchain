...
- **THEN** the system returns the brief with its current final version content, `version` equals `current_version`, `is_current` is `true`, and `unfinalized_version` is the version number of any editable draft if one exists, otherwise `null`
...
- **THEN** the system returns the brief with version 1 content, `version` equals 1, `is_current` is `true`, and `unfinalized_version` is 2

#### Scenario: Get historical version
- **WHEN** an authenticated user sends a GET request to `/api/v1/briefs/:brief_id?version=1`
- **THEN** the system returns the brief content for version 1, `is_current` is `true` only when version 1 equals `current_version`, and `unfinalized_version` is the version number of any editable draft if one exists, otherwise `null`
