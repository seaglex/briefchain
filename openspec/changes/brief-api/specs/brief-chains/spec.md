...
The system SHALL return a list of brief chains accessible to the current user.

#### Scenario: Successful chain list
- **WHEN** an authenticated user sends a GET request to `/api/v1/chains`
- **THEN** the system returns chains with chain_id, title, root_brief_id, brief_count, and created_at
...
- **THEN** the system returns the chain metadata, root brief in detail mode, and the full brief tree
