...
The system SHALL allow the creator to send a reviewed brief to a downstream user, transitioning `upstream_state` to "sent".
...
- **THEN** the system sets `briefs.upstream_state` to "in_process" and `downstream_state` to "opened", updates `status_changed_at` / `status_changed_by`, updates the latest pending transfer record with `accepted_at`, and returns the updated brief and transfer
...
- **THEN** the system sets `briefs.upstream_state` to "editing", clears `assigned_to` and `assigned_to_name`, updates `status_changed_at` / `status_changed_by`, updates the latest pending transfer record with `rejected_at` and `rejection_reason`, and returns the updated brief and transfer
