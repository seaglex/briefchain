...
**Goals:**
- 基于 FastAPI 实现 `/api/v1/briefs` 路由：创建、列表、详情（含 `unsent_version`）、editing 阶段操作（patch / submit-review）。
...
- **版本管理**：
  ...
  - 详情接口通过 `?version=` 参数返回指定版本内容，默认返回 `current_version` 内容；同时返回 `unsent_version` 字段标识可编辑 draft（null 表示无）。
- **状态流转**：
...
- **响应模式**：
  ...
  - 详情模式：列表模式 + `root_id`, `parent_id`, `content`, `attachments`, `current_version`, `version`, `is_current`, `unsent_version`, `estimated_man_days`, `expected_completion_at`, `created_at`。
