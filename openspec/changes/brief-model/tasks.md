...
- [x] 4.1 Implement `Brief` model in `src/briefchain/models/brief.py` with dual state fields and redundant name/title/priority/tracking fields
- [x] 4.2 Implement self-referential `parent` / `children` relationships on `Brief`
- [x] 4.3 Implement `BriefVersion` model with composite primary key `(brief_id, version)`, lifecycle status, and arbiter review reference
- [x] 4.4 Implement `BriefTransferHistory` model mapped to `brief_transfer_history` with sent/accepted/rejected timestamps and sender/receiver name snapshots
- [x] 4.5 Implement `BriefChain` model with `chain_id` as primary key and owner/priority redundant fields
- [x] 4.6 Implement `BriefArbiterReview` model with JSON `issues` and `suggestions`
- [x] 4.7 Wire `Brief` relationships to versions, transfers, chain, feedback, and reviews
- [x] 4.8 Add `lazy="raise"` to all `Brief` and related model relationships

## 5. Feedback Models

- [x] 5.1 Implement `Feedback` model in `src/briefchain/models/feedback.py` with `is_to_down`, sender/receiver name snapshots, and direction-aware type
