...
The system SHALL provide a `BriefTransferHistory` SQLAlchemy model mapped to the table `brief_transfer_history` that records the brief identifier, brief version, associated arbiter review, sender, receiver, sent time, accepted time, rejected time, and rejection reason.
