...
def test_generate_and_verify_invite_token() -> None:
    ...
    token = invite_service.generate_invite_token(brief_id, nonce, deadline)
    parsed_brief_id, parsed_nonce, parsed_epoch = invite_service.verify_invite_token_signature(
        token
    )
...
def test_sign_payload_uses_jwt_secret(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    token_a = invite_service.generate_invite_token(brief_id, nonce, deadline)
    assert invite_service.verify_invite_token_signature(token_a)

    monkeypatch.setattr(settings, "jwt_secret_key", "different-secret")
    token_b = invite_service.generate_invite_token(brief_id, nonce, deadline)
