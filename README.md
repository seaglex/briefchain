...
传统工具的问题
- 工具设计面向需求方，经常由于需求描述不够清楚而导致执行偏差、返工等问题，反而影响需求达成
- 状态需要人工维护，记录可靠性差
- 项目负责人（如果不是产研）不用，只记录部分项目周期，进度对负责人透明度不够
...
1. **平等对待 upstream (通常是需求方) 和 downstream (通常是执行方)**，以 AI 审核保证需求质量，以闭环复盘能力优化组织
2. **以 brief 的转移为工作天然分界**，无需人工维护重要项目阶段，自动沉淀可靠文档
3. **全链路透明闭环**，自动聚合 downstream 进展，保证所有 upstreams 随时了解工作状态
...
Feedbacks 是合约期间的**正式合同通知**，不是聊天消息。
包含两类
- Downstream 的项目进度更新、状态更新
- Upstream 的状态更新（暂停、取消、审核）

Brief分配后，Brief 的 upstream 和 downstream 的状态分别如下，并通过 feedbacks 更新
- upstream状态：in_process, suspended, cancelled, done
- downstream状态：opened, delegated, blocked, submitted
