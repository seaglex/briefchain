...
> **版本行为（按请求中的 `version` 校验）：**
> - 请求 `version` 存在且为当前最新未终结版本（draft / reviewed）→ 原地修改该版本
> - 请求 `version` 不存在但等于 `max_version + 1`，且当前无未终结版本 → 从当前最终版本 fork 出新 draft
> - 其他情况（版本不存在且不是下一个、或不是最新未终结版本）→ 返回 409 `VERSION_CONFLICT` / 404 `VERSION_NOT_FOUND`
> - 原地修改时：draft 保持 draft；reviewed 重置为 draft
...
// Request
{
  ...
  "version": 1   // 必填，必须是当前最新未终结的 draft 版本
}
...
> user_id 从 JWT 解析，校验 = created_by，且 upstream_state 不是 cancelled/done
> 版本状态：请求指定的 `version` 必须存在、是当前最新未终结版本、且 status 为 draft → 转为 reviewed（arbiter_review_id 填入 brief_versions）
> 若版本不存在、不是最新未终结版本、或 status 不是 draft，返回 409 / 404
...
{
  "action": "update",
  "version": 2,                         // 必填，要推送的 reviewed 未终结版本号
...
> **版本流转（MVP: auto-pass）：**
> 1. 校验请求指定的 `version` 必须属于该 brief 且 status 为 reviewed
> 2. 将该 reviewed 版本提升为 final
> 3. briefs.current_version = 该版本号，briefs.title/priority 同步
> 4. downstream_state → opened（强制重开，需求变了下游必须基于新版本重新执行）
> 5. 创建 feedback (type=update, is_to_down=true)
...
> 所有 action 共用相同的 Request/Response 结构，差异仅为 `action` 值；`content` 对所有 action 均必填，仅 `attachments` 可选。
