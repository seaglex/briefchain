...
> 最后更新：2026-06-27（v4：冗余存储 + API 聚类）
...
```
editing ──(send)──→ sent
                      ↓ accept (downstream)       ↓ reject (downstream)
                 (in_process, opened)          → editing
...
```
current_version 和 patch 动作
- 如果 current_version 是 None，直接修改 brief_version
- 如果 current_version 不是 None，新增一个 brief_version
- send 触发 brief.current_version 和 version 关联上
...
Brief 是合约，操作是审慎的。前端根据当前状态组合 + 当前用户角色，显示可用动作按钮：

```
[upstream=editing] [upstream 视角]
  → [提交审查 → reviewed] [编辑 PATCH]
...
[upstream=in_process, downstream=delegated] [downstream 视角]
  → [收回 → opened] [标记阻塞 → blocked] [提交完成 → submitted]

[upstream=in_process, downstream=submitted] [upstream 视角]
  → [验收通过 → (done, null)] [打回 → (in_process, opened)] [推送更新 → (in_process, opened)]
...
每个动作点击后可能弹出确认框或表单（比如 submit 要求填完成说明，block 要求填阻塞原因，reject_submit 要求填打回原因）。
...
[upstream send]       → v1: sent（briefs.current_version = 1，title/priority/expected_completion_at 同步）
                       upstream_state = sent, downstream_state = null

[downstream accept]   → upstream_state = in_process, downstream_state = opened

[upstream update 推送] → v2: draft → auto-reviewed → auto-sent
                       briefs.current_version = 2，title/priority 同步
...
- update 走完整 Arbiter 审查（draft → reviewed → sent），与初始 send 一致（MVP: auto-pass）
...
| true | approve | → done | → null | upstream 验收通过，终态 |
| true | reject_submit | 不变 | → opened | upstream 打回提交 |
| true | update | 不变 | → opened | upstream 推送新版本，强制下游重开 |
...
| 状态变更 | 发起方 | 记录位置 |
|---------|--------|---------|
| (editing, null) → (sent, null) | upstream: send | brief_transfer_history |
...
| (in_process, submitted) → (done, null) | upstream: approve | feedbacks (type=approve, is_to_down=true) |
